                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    Point3 first;
    ScreenPtToGround(view_matrix,proj_matrix, stroke2d.front(), &first);
    Point3 last;
    ScreenPtToGround(view_matrix,proj_matrix, stroke2d.back(), &last);
   // ((stroke2d.front())[0],(stroke2d.front())[1],0);
  //  Point3 last = Point3 ((stroke2d.back())[0],(stroke2d.back())[1],0);
    Vector3 plv = last - first;
    Vector3 norm = plv.Cross(Vector3(0,1,0));
    norm.Normalize();
    ...
    for (int i=0; i<stroke2d.size(); i++) {
        Point3 sp = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, stroke2d[i]);
        Ray ray (eye,(sp - eye).ToUnit());
        
        Point3 p1;
        float time;
        ray.IntersectPlane(first, norm,&time, &p1);
        verts.push_back(p1);
    }
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
     ...
     //   Vector3 v1 = first - P;
      //  double d = v1.Dot(norm) / (norm.Length());
       double d =  P.DistanceToPlane(first, norm);
       Point3 p2 =  P.ClosestPointOnPlane(first, norm);
        if(std::max(0.0,1.0 - pow(d/5.0,2.0)) != 0){
        if(hfunc(norm, verts, p2) > 0.0){
            P[1] = (1 - std::max(0.0,1.0 - pow(d/5.0,2.0)))*P[1]+ std::max(0.0,1.0 - pow(d/5.0,2.0))*hfunc(norm, verts, p2);
        }
        }
